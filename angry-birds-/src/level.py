...
class Level():
    def __init__(self, pigs, columns, beams, space):
        'impostiamo gli oggetti presenti nel livello'
        self.pigs = pigs
        ...
        self.bool_space = False #difficolta
        self.wood_images = WoodImages()

    #funzione per creare una strturra pre impostata
    def open_flat(self, x, y, n):
        ...
        for i in range(n):
            y = y0+100+i*100
            p = (x, y)
            self.columns.append(Polygon(p, 20, 85, self.space,wood_images=self.wood_images))
            p = (x+60, y)
            self.columns.append(Polygon(p, 20, 85, self.space,wood_images=self.wood_images))
            p = (x+30, y+50)
            self.beams.append(Polygon(p, 85, 20, self.space,wood_images=self.wood_images))
    ...
    def build_0(self):
        """level 0"""
        pig1 = Pig(980, 100, self.space)    #personaggi da inserire nel livello
        pig2 = BeardedPig(985, 182, self.space)
        self.pigs.append(pig1)
        self.pigs.append(pig2)
        p = (950, 80)   #poszione delle colonne/travi
        self.columns.append(Polygon(p, 20, 85, self.space,wood_images=self.wood_images)) # p -> posizone, 20-> lunghezza, 85-> altezza della colonna
        p = (1010, 80)
        self.columns.append(Polygon(p, 20, 85, self.space,wood_images=self.wood_images))
        p = (980, 150)
        self.beams.append(Polygon(p, 85, 20, self.space,wood_images=self.wood_images))   # p -> posizone, 85 ->lunghezza, 20-> altezza della trave
        p = (950, 200)
        self.columns.append(Polygon(p, 20, 85, self.space,wood_images=self.wood_images))
        p = (1010, 200)
        self.columns.append(Polygon(p, 20, 85, self.space,wood_images=self.wood_images))
        p = (980, 240)
        self.beams.append(Polygon(p, 85, 20, self.space,wood_images=self.wood_images))
        self.number_of_birds = 4    #impostiamo il numero di bird disponibile
        if self.bool_space: # se vogliamo semplificare il livello impostiamoad 8
    ...
    def build_12(self):
        """Level 12"""
        pig = Pig(800,80,self.space)
        pig.life = 40
        self.pigs.append(pig)
        pig = Pig(820,80, self.space)
        pig.life = 40
        self.pigs.append(pig)
        pig = Pig(890,80,self.space)
        pig.life = 60
        self.pigs.append(pig)

        self.horizontal_pile(700,0,6)
        p = (790, 80)
        self.columns.append(Polygon(p,20,85,self.space))
        p = (850, 80)
        self.columns.append(Polygon(p,20,85, self.space))
    ...
    def build_13(self):
        """Level 13"""
        pig = Pig(560,60,self.space)
        pig.life = 100
        self.pigs.append(pig)
        self.vertical_pile(520,0,2)
        self.vertical_pile(580,0,2)
        self.horizontal_pile(550,200,4)
        self.horizontal_pile(440,0,4)
        self.horizontal_pile(640, 0,4)


    #funzione per il caricamento deei livelli
    #passiamo il nome + la variabile number che viene settata nel main
    # in caso di numero di livello errato si setta a 0
    def load_level(self):
        try:
